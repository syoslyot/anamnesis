...
function buildContext(anamnesisDir) {
  ...
  if (concluded.length) {
    sections.push('Recent Conclusions:\n' + concluded.map(e => {
      const icon = e.conclusion.startsWith('✅') ? '✅'
                 : e.conclusion.startsWith('❌') ? '❌' : '🔄';
      return `  ${icon} [${e.meta.id}] ${e.conclusion || e.firstLine}`;
    }).join('\n'));
  }
...
function parseFile(filePath) {
  try {
    ...
    const firstLine = bodyLines
      .find(l => !l.startsWith('#'))
      ?.replace(/^##?\s*/, '').trim() || '';

    const conclusionIdx = body.indexOf('## 結論\n');
    const conclusion = conclusionIdx !== -1
      ? body.slice(conclusionIdx + 6).split('\n').find(l => l.trim())?.trim() || ''
      : '';
